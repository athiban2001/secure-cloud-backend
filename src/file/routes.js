...
fileRouter.post(
	...
	async (req, res) => {
		...
		try {
			...
			returnData.storage_filename = `${req.params.group_id}-${
				req.payload.id
			}-${nanoid()}`;

			const parts = req.body.original_filename.split(".");
			parts.forEach((part, index) => {
				if (index != 0) {
					returnData.storage_filename += "." + part;
				}
			});
			returnData.storage_filename += ".enc";
			console.log(returnData.storage_filename);
			...
			if (response.rows[0].name_count == 0) {
				...
				returnData.original_filename =
					parts[0] + `(${response.rows[0].name_count})`;
				parts.forEach((part, index) => {
					if (index != 0) {
						returnData.original_filename += "." + part;
					}
				});
			}
			response = await client.query(
...
fileRouter.post(
	...
	async (req, res) => {
		...
		try {
			await client.query("BEGIN");
			response = await client.query(
				"SELECT * FROM files WHERE id=$1 AND user_id=$2 AND group_id=$3",
			...
			fs.writeFileSync(
				path.join(
					__dirname,
					"../uploads",
					response.rows[0].storage_filename
				),
				Buffer.from(req.body.content, "base64").toString("binary")
			);
			await client.query(
